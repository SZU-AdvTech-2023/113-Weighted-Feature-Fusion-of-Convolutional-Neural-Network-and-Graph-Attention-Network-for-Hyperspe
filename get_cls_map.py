...
def list_to_colormap(x_list):
    ...
    for index, item in enumerate(x_list):
        ...
        if item == 17:
            y[index] = np.array([156, 189, 255]) / 255.
...
def test(device, net, test_loader):
    ...
    y_pred_test = 0
    y_test = 0
    for inputs_hsi, inputs_lidar, labels in test_loader:
        inputs_hsi, inputs_lidar = inputs_hsi.to(device), inputs_lidar.to(device)
        outputs = net(inputs_hsi, inputs_lidar)
...
def get_cls_map(y_pred, y, j):
    ...
    classification_map(y_re, y, 300,
                       'classification_maps/' + 'IP_predictions_pav{}.png'.format(j))
    if (j == 0):
        classification_map(gt_re, y, 300,
                           'classification_maps/' + 'IP_gt_pa.png')
